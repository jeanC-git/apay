  ...
  <div>
    ...
    <v-row class="mt-5" v-if="arrayProductos.length>0">
      <v-col cols="12" md="3" sm="6" v-for="producto in arrayProductos" :key="producto.id">
        <v-card class="mx-auto" max-width="100%" min-height="450">
          <v-img
            class="white--text align-end"
            height="200px"
            :src="'images/productos/'+producto.foto"
          >
            <v-card-title v-if="producto.stock==0" v-text="'No disponible'"></v-card-title>
          </v-img>
          ...
          <v-card-actions class="d-flex justify-center" v-if="producto.stock>0">
            <v-btn color="orange" text @click="dialog_stock=true">Añadir</v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
