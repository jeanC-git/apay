<template>
  <div height="100%" width="100%">
    <v-app-bar color="#ffff" absolute>
      ...
      <v-col cols="12"  xs="3" sm="2" md="2" class="d-flex justify-end">
      ...
      <v-row v-if="arrayProductos.length>0" class="d-flex justify-start">
        <v-col style="height:50%" cols="12"   xs="12" sm="6" md="3" v-for="producto in arrayProductos" :key="producto.id">
          <v-card width="100%" class="mx-auto" color="green accent-3">
            ...
            <v-img :src="producto.foto" contain aspect-ratio="1"></v-img>
            <v-card-text>
          ...
          <v-list three-line subheader >
            <v-list style="overflow:auto" min-width="600px">
              <v-list-item-group color="primary">
                ...
                <v-list-item v-for="(producto_lista,index) in carrito_compras" :key="producto_lista.id" style="max-height:20px;padding:0">
                  <v-row >
                    <v-col  cols="12" sm="1" xs="1" md="1" class="d-flex justify-center align-center">
                    ...
                    <v-col cols="12" sm="1" xs="1" md="1" class="d-flex justify-center align-center">
                      <div v-text="producto_lista.nombre_puesto+' - '+ producto_lista.numero_puesto"></div>
                    </v-col>
                    <v-col  cols="12" sm="3" xs="3" md="3" class="d-flex justify-center align-center">
                    ...
                    <v-col  cols="12" sm="2" xs="2" md="2" class="d-flex justify-center align-center">
                      ...
                      <div style="max-width:60px;">
                        <v-text-field style="text-align:center" @keyup="modificar_lista(index,'mayor')" v-model="producto_lista.cantidad" class="mx-3" ></v-text-field>
                      </div>
                    ...
                    <v-col  cols="12" sm="2" xs="2" md="2" class="d-flex justify-center align-center">
                      <div v-text="'S/.'+Math.round(producto_lista.precio*producto_lista.cantidad * 100) / 100"></div>
                    </v-col>
                  </v-row>
                </v-list-item>
              </v-list-item-group>
...
export default {
  ...
  methods: {
    ...
    añadir_carrito(producto) {
      let me = this;
      let valor;
      const found = me.carrito_compras.find(element => element.id == producto.id);
      console.log(found);
      if(found==undefined){
      ...
      switch (tipo) {
        ...
        case 'eliminar':
            Swal.fire({
              ...
              if (result.value) {
                const found = me.arrayProductos.find(element => element.id == me.carrito_compras[producto_index].id);
                found.cantidad=1;
